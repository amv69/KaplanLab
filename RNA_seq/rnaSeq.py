...
"""
BIG DISCLAIMER: TO-DO

I wrote this to merge bed files here, it should be down prior to bigwig conversion (probably)

"""

#Sorts the bedgraph files to ensure bedGraphToBigWig works correctly
subprocess.check_call("for file in *.bed; do echo $file && LC_COLLATE=C sort -k1,1 -k2,2n $file > ${file/.bed/_sorted.bed}; done", shell=True)

#Here it is ^
subprocess.check_call("for file in *.bed; do echo $file && bedtools merge -i $file -s -c 6 -o distinct  > ${file/.bed/_merged.bed}; done", shell=True)

#Get genomecoverage for bed files
subprocess.call('for str in "+" "-"; do [ "$str" = "+" ] && n="rev" || n="fw"; for file in *_merged.bed; do sample=${file} && echo $n $sample && bedtools genomecov -g /bgfs/ckaplan/Anand_seq/Genomes/Combined_yeast/Budding_Fission_new.genome -i $file -bg -5 -strand $str > ${sample}_${n}.bg; done; done', shell=True)
...
subprocess.check_call("for file in *.bedgraph; do echo $file && awk '$1 ~ /chr/' $file > ${file/.bedgraph/.bedgraph}; done", shell=True)

#Sorts the bedgraph files to ensure bedGraphToBigWig works correctly
subprocess.check_call("for file in *.bedgraph; do echo $file && LC_COLLATE=C sort -k1,1 -k2,2n $file > ${file/.bedgraph/_sorted.bedgraph}; done", shell=True)

#Conversion to BigWIg
subprocess.check_call("for file in *.bedgraph; do echo $file && bedGraphToBigWig $file /bgfs/ckaplan/Anand_seq/Genomes/Combined_yeast/Budding_Fission_new.genome $file.bw", shell=True)
